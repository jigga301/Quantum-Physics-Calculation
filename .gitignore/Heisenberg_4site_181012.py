...
Bit operations	
"""
def SetBit(i,n):#把第n位改为1（i转化为二进制）
	return i|(1<<n)

def ClearBit(i,n):#把第n位改为0
	return i&~(1<<n)

def FlipBit(i,n):#翻转第n位置
	return i^(1<<n)

def ReadBit(i,n):#读取第n位的01
	return (i&(1<<n))>>n

def PopCntBit(i):#数有多少个1
	return bin(i).count("1")

def PickBit(i,k,n):#从第k位开始选择第n位的01
	return (i&((2**n-1)<<k))>>k
...
def GetBasisK(Ns):
	...
	Check = np.zeros((Nl,3),dtype=int) #check 中第0位代表是不是principal-configuration，第1位代表principal矩阵，第2位代表transation-step
	for i in range(Nl):
		if Check[i,0] != 1:
			PriC.append(i)
			Check[i,1] = PriC[-1]
...
#----------------------------------------------------------------
	...
	for i0 in range(Nl):
		for ih in range(len(HopList)):
			...
			HI.append(i0)
			HJ.append(i0)
			HV.append((ReadBit(i0, Pos0) - 0.5) * (ReadBit(i0, Pos1) - 0.5) * 4)

	Hamr = sparse.coo_matrix((HV, (HI, HJ)), shape=(Nl, Nl)).tocsc()
	# print Hamr.todense()

	S, V = LA.eigh(-J * Hamr.todense())
	print('(2) do not apply any symmetry')
	# print S
	...
	"""
    (3) apply translational symmetry
    """
	...
	for k in range(Ns):
		if len(Basis[k]) > 0:
				...
				for ih in range(len(HopList)):
					...
					if 1:
						...
						try:
							...
							HI.append(j1)
							HJ.append(j0)
							HV.append(g * np.sqrt(float(i0PriR) / float(i1PriR)) * np.exp(
								1j * 2 * math.pi * k / float(Ns) * i1PriL))
						# print j0,i0,np.binary_repr(i0,Ns),Pos0,Pos1,np.binary_repr(i1,Ns),i1,i1PriC,i1PriL,i1PriR,i0PriR,j1,HV[-1]
						except ValueError:
							pass
			...
			S, V = LA.eigh(-J * Hamk.todense())
			# print S
			Spm3 = np.append(Spm3, S)

	print('(3) apply translational symmetry')
	Spm3 = np.sort(Spm3)
	print(Spm3)

	print('Check different methods')
	print(LA.norm(Spm2 - Spm1), LA.norm(Spm3 - Spm1))
